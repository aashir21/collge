...
const LinkUpByUser = () =>
{
    ...
    const fetchUserLinkUps = async () =>
    {
        ...
        try
        {
            ...
            const response = await customFetch(`${ENDPOINT.BASE_URL}/api/v1/linkup/getAllLinkUpsByAuthorId?authorId=${currentUser}&offset=${offset.current}&pageSize=${PAGE_SIZE}`)
            if (response.ok)
            {
        ...
        <View style={styles.container}>
            {
                linkups.length > 0 ?
                    ...
                    <View style={{ height: "100%", justifyContent: "center", alignItems: "center", alignSelf: "center" }}>
                        <Text style={styles.emptyTitle}>You have not created any <SpanText subtext={"LinkUps"} /></Text>
                    </View>
            }
        </View>
    )
