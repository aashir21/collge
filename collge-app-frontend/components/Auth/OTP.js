...
const OTP = ({ email }) =>
{
    ...
    const handleVerifyOtp = async () =>
    {
        ...
        try
        {
            ...
            const response = await fetch(`${ENDPOINT.BASE_URL}/api/v1/otp/verifyOTP?userId=${localSearchParams.userId}&otpCode=${otp}`)
            ...
            if (response.ok)
            {
                router.replace("/auth/confirmpassword")
                router.setParams(localSearchParams)
                setError({})
            }
            ...
            <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
                <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"}>
                    ...
                    <View style={styles.fieldContainer}>
                        <Text numberOfLines={2} style={[styles.title, { width: width - 32 }]}>OTP Verification</Text>
                        ...
                        <TouchableOpacity disabled={isSubmitDisabled} style={[styles.resendBtn, { opacity: isSubmitDisabled ? 0.2 : 1 }]} onPress={handleValidation}>
