...
const CommentMetion = ({ username, onClose, initialMentionUsers, handleSetMentionedUser, input, atIndex }) =>
{
    ...
    useEffect(() =>
    {
        if (input.substring(atIndex) !== null)
        {
            handleMentionChange(input.substring(atIndex))
    ...
    const handleMentionChange = (text) =>
    {
        ...
        handleSearch(input.slice(atIndex + 1));
    }
