...
const LinkUpSoloProfileHeader = ({ name, profilePicUri, value, isPremiumUser, role, userId, createdAt, showModal, location, taggedUsers }) =>
{
            ...
            <View style={{ justifyContent: "center", alignItems: "center", flexDirection: "row" }}>
                ...
                <View style={{ flexDirection: "column" }}>
                    ...
                    <View style={{ flexDirection: "row" }}>
                        <Text numberOfLines={1} style={[styles.postAuthorName, { fontFamily: FONT.regular, fontSize: SIZES.small + 1, color: COLORS.secondary }]}>{value?.length > 12 ? value?.substring(0, 20) + ".." : value}</Text>
                        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "flex-start" }}>
                            ...
                            }
                            {
                                location && <Pressable onPress={openShowLocationModal} style={[styles.locationContainer]}>
                                    ...
                                    <Text style={[styles.locationName]}>{location?.length > 22 ? location?.substring(0, 18) : location}</Text>
                                </Pressable>
                            }
                        </View>
                    </View>
...
export default React.memo(LinkUpSoloProfileHeader)
