...
const TaggedUserTab = ({ userId, avatar, firstName, lastName, username, onClose, premiumUser, role, bgColor, handleRemoveTaggedUser, shouldRunFunction }) =>
{
    ...
    return (
        <Pressable onPress={handleCustomOnPress}>
            <View style={[styles.container, { width: width - SIZES.large, flexDirection: "row", backgroundColor: bgColor }]}>
                ...
                {
                    shouldRunFunction && <View style={{ justifyContent: "center", alignItems: "flex-end" }}>
                        <AntDesign name="minus" size={16} color={COLORS.error} />
                    </View>
                }
            </View>
        </Pressable>
